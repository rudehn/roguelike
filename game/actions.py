...
class Melee:
    ...
    def __call__(self, entity: tcod.ecs.Entity) -> ActionResult:
        ...
        damage_result = melee_damage(entity, target)
        ...
        match damage_result.action_type:
            case CombatActionTypes.BLOCKED:
                add_message(entity.registry, f"{attack_desc} but does no damage.", attack_color)
            case CombatActionTypes.MISSED:
                add_message(entity.registry, f"{attack_desc} but missed.", attack_color)
            case CombatActionTypes.CRIT:
                add_message(entity.registry, f"{attack_desc} and crits for {damage_result.damage} hit points!", attack_color)
                apply_damage(target, damage_result.damage, blame=entity)
            case CombatActionTypes.ATTACK:
                add_message(entity.registry, f"{attack_desc} for {damage_result.damage} hit points.", attack_color)
                apply_damage(target, damage_result.damage, blame=entity)
